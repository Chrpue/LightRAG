...
# 兼容容器名：postgres/neo4j 以及 lightrag-postgres/lightrag-neo4j
...
# --- 彩色输出与格式化工具 ---
if tput setaf 2 >/dev/null 2>&1; then
...
info(){ printf "\n%b>>> %s%b\n" "$GREEN" "$1" "$RESET"; }
say(){  printf "%s\n" "$1"; }
ask(){  printf "%b%s%b" "$YELLOW" "$1" "$RESET"; }
ok(){   printf "%b✓ %s%b\n" "$GREEN" "$1" "$RESET"; }
fail(){ printf "%b✕ %s%b\n" "$RED"   "$1" "$RESET"; }

# 新增：带缩进的输出函数
say_i(){  printf "%s%s\n" "$INDENT" "$1"; }
ask_i(){  printf "%s%b%s%b" "$INDENT" "$YELLOW" "$1" "$RESET"; }
...
compose_down(){
  ...
  # 隐藏 compose down 可能出现的无关错误信息
  docker compose -p "$PROJECT_NAME" "${args[@]}" down 2>/dev/null || true
  popd >/dev/null
}
compose_up(){
  pushd "$COMPOSE_DIR" >/dev/null
...
# 获取容器 ID（先尝试 compose service，再按容器名模糊匹配）
get_cid_by_names(){
  local name cid
  for name in "$@"; do
...
say_i "请输入项目文件存放的根目录，所有数据将被存储在 ${YELLOW}<您输入的目录>/lightrag${RESET} 下。"
ask_i "路径 (默认: ${BASE_DIR_DEFAULT}): "
read -r BASE_DIR
...
if [[ -d "$LR_ROOT" ]] || [[ -d "$LR_PG" ]] || [[ -d "$LR_NEO4J" ]] || [[ -d "$LR_INPUTS" ]] || [[ -d "$LR_STORAGE" ]]; then
  ...
  read -r wipe
  if [[ "$wipe" =~ ^[yY]$ ]]; then
    ask_i "为防误删，请输入 ${RED}DELETE${YELLOW} 确认（其他任意键取消）："
    read -r del2
    if [[ "$del2" == "DELETE" ]]; then
...
if [[ "$wipev" =~ ^[yY]$ ]]; then
  for v in "${VOLUMES[@]}"; do
    docker volume inspect "$v" >/dev/null 2>&1 && docker volume rm "$v" >/dev/null || true
